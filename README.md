...
| 頁面 | 用途 |
|---|---|
| `index.html` | **老師管理**：登入、建立每週簽到、投影 QR Code、即時名單、發問計數（＋／－）、隨機抽問、關閉／重新開放簽到、匯出本週／全學期 CSV |
...
4. 學生發問時，老師在該生列按「＋」；按錯按「－」。
   **隨機抽問**：按「🎲 隨機抽問」會從本週已簽到名單隨機抽出一位（名字滾動後停下），可「再抽一位」；勾選「本週已抽過的不重複」則同一週抽完一輪前不會重複；「＋1 發問並完成」會直接幫該生記一次發問。
